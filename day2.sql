...
select *
from  regexp_split_to_table($$2-5 z: zzztvz
...
1-11 t: tfvtqvlbtld
4-5 k: kkkczkkkvkkk
2-7 p: ptphppvppppp$$, E'\n') WITH ORDINALITY as t(entry, id)
);


drop table if exists parsed_inputs;
create temp table parsed_inputs as (
select id
	,entry
	,substring(requirements[1] from '(\d+)-\d+') as minimum
	,substring(requirements[1] from '\d+-(\d+)') as maximum
	,left(requirements[2], 1) as letter
	,requirements[3] as pword
from (
select i.*
	,regexp_split_to_array(entry, ' ') as requirements
from inputs i
)a
)
...
from(
select *
	,case when char_length(pword) - char_length(replace(pword, letter, '')) 
		between minimum::int and maximum::int then 'yes'::bool else 'no'::bool end as password_valid
from parsed_inputs
)a
where password_valid

--part2
select *
from(
select *
	,case when (substr(pword, minimum::int, 1)=letter)::int + (substr(pword, maximum::int, 1) = letter)::int = 1 
